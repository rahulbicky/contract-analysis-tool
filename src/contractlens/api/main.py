import sys
import os
# Add 'src/' to path so that 'contractlens' package is importable
...
async def analyze_contract(request: Request, file: UploadFile = File(...)):
    ...
    # Save uploaded file
    upload_path = f"./data/raw/upload_{thread_id}.pdf"
    ...
    parsed_path = f"./data/processed/upload_{thread_id}_parsed.json"
    collection = f"contractlens_{thread_id}"
    bm25_path = f"./data/processed/{thread_id}_bm25.pkl"
    bm25_meta_path = f"./data/processed/{thread_id}_bm25_meta.json"
    ...
    try:
        ...
        chunks = chunk_document(parsed_path, output_dir="./data/processed")
